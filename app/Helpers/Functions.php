...
function newslist($id,$limit='',$offset=''){
 $newstype_id = $id;
 if($limit){
   $data = NewsTypeModel::find($newstype_id)->news()->where(['status'=>'1'])->orderBy('ordering','desc')->offset($offset)->limit($limit)->get();
 }else{
   $data = NewsTypeModel::find($newstype_id)->news()->where(['status'=>'1'])->orderBy('ordering','desc')->get();
 }
 if($data->count() > 0){
  return $data;     
}
...
function newstop($id){
  $newstype_id = $id;
  $data = NewsTypeModel::find($newstype_id)->news()->where(['status'=>'1'])->orderBy('ordering','desc')->first();
  if($data <> NULl ){
    return $data;     
  }
  return false;
...
function is_empty_team($id){
$data = ContentWriterModel::where(['email'=>$id])->count();
  if( $data > 0){
    return true;
  }
  return false;
...
function team_cat($id){
$data = teamcategoryModel::where(['id'=>$id])->first();
return $data;
}

function designation($id){
$data = ContentWriterModel::where('id',$id)->first();
$team = teamcategoryModel::where('id',$data->email)->first();
 if($team <> NULL ){
    return $team;     
  }
  return false;
